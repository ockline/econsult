    ...
    return (
		...
		<div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
				...
				<ol className="flex items-center whitespace-nowrap min-w-0 text-end">
					...
					<li className="text-sm">
					<a className="flex items-center text-primary hover:text-primary dark:text-primary" href={`${import.meta.env.BASE_URL}leaves/maternity`}>
						Leave Management
						{/* <i className="ti ti-chevrons-right flex-shrink-0 mx-3 overflow-visible text-gray-300 dark:text-white/10 rtl:rotate-180"></i> */}
					</a>
					</li>
				...
				<div className="col-span-12">
					<div className="box">
						<div className="box-header">
							<div className="flex">
								<h5 className="box-title my-auto">Employees Maternity Leave</h5>
								<div className="space-y-2">
                                     	<Link to={`${import.meta.env.BASE_URL}leaves/maternity/create_maternity_leave/`}>
											...
											<td>
												<div className="flex space-x-3 rtl:space-x-reverse w-full">
													<div className="block w-full my-auto">
														<span
															className="block text-sm font-semibold text-gray-800 dark:text-gray-300 min-w-[180px] truncate">{maternity.employee_id}</span>
															</div>
												</div>
                                                    </td>
                                                    <td>{maternity.employee_name}</td>
											<td className="!text-success font-semibold text-base">{maternity.employer}</td>
											<td>
												<div className="flex space-x-3 rtl:space-x-reverse text-start">
													
													<div className="block my-auto">
														<p className="block text-sm font-semibold my-auto text-gray-800 dark:text-white">{maternity.leave_type}</p>
														{/* <span
															className="block text-xs text-gray-400 dark:text-white/80 !font-normal my-auto">socratesitumay@gmail.com</span> */}
													</div>
												</div>
                                                    </td>
                                                    <td>{maternity.start_date}</td>
												<td>{maternity.end_date}</td>                                                    
											<td>{maternity.all_balance}</td>
													<td>
														{ maternity.status === 1
														? (
<span
												className="truncate whitespace-nowrap inline-block py-1 px-3 rounded-full text-xs font-medium bg-success/10 text-success/80">Submitted</span>)													
												 : (<span
															className="truncate whitespace-nowrap inline-block py-1 px-3 rounded-full text-xs font-medium bg-danger/15 text-info/80">{maternity.status}</span>
												 )}
											</td>
					
											<td className="font-medium space-x-2 rtl:space-x-reverse">
												<div className="hs-tooltip ti-main-tooltip">
													<Link to={`${import.meta.env.BASE_URL}leaves/maternity/show_maternity_leave/${maternity.id}`}
													...
													&nbsp;&nbsp;
													{/* <Link to={`${import.meta.env.BASE_URL}leaves/maternity/editLeave/${maternity.id}`} className="ti-btn ti-btn-primary me-1">
                                                    ...
                                                    onClick={() => {
                                                   navigate(`${import.meta.env.BASE_URL}leaves/maternity/edit_maternity_leave/${maternity.id}`);   
                                                    }}
                                                   >
                                                    <i className="ti ti-pencil"></i>
                                                </button>			
										      &nbsp;&nbsp;														
											{maternity.status === null ? (
										<button
											type="button"
											className="ti-btn ti-btn-success show-example-btn"
											aria-label="Deactivate! Example: End of contract"
											id="ajax-btn"
											onClick={(e) => Ajaxcalling(e, maternity.id)}
										>
											Initiate
										</button>
										) : (
