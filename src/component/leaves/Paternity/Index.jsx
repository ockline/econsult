    ...
    return (
		...
		<div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
				...
				<ol className="flex items-center whitespace-nowrap min-w-0 text-end">
					...
					<li className="text-sm">
					<a className="flex items-center text-primary hover:text-primary dark:text-primary" href={`${import.meta.env.BASE_URL}leaves/paternity`}>
						Leave Management
						{/* <i className="ti ti-chevrons-right flex-shrink-0 mx-3 overflow-visible text-gray-300 dark:text-white/10 rtl:rotate-180"></i> */}
					</a>
					</li>
				...
				<div className="col-span-12">
					<div className="box">
						<div className="box-header">
							<div className="flex">
								<h5 className="box-title my-auto">Employees Paternity Leave</h5>
								<div className="space-y-2">
                                     	<Link to={`${import.meta.env.BASE_URL}leaves/paternity/create_paternity_leave/`}>
											...
											<td>
												<div className="flex space-x-3 rtl:space-x-reverse w-full">
													<div className="block w-full my-auto">
														<span
															className="block text-sm font-semibold text-gray-800 dark:text-gray-300 min-w-[180px] truncate">{paternity.employee_id}</span>
															</div>
												</div>
                                                    </td>
                                                    <td>{paternity.employee_name}</td>
											<td className="!text-success font-semibold text-base">{paternity.employer}</td>
											<td>
												<div className="flex space-x-3 rtl:space-x-reverse text-start">
													
													<div className="block my-auto">
														<p className="block text-sm font-semibold my-auto text-gray-800 dark:text-white">{paternity.leave_type}</p>
														{/* <span
															className="block text-xs text-gray-400 dark:text-white/80 !font-normal my-auto">socratesitumay@gmail.com</span> */}
													</div>
												</div>
                                                    </td>
                                                    <td>{paternity.start_date}</td>
												<td>{paternity.end_date}</td>                                                    
											<td>{paternity.all_balance}</td>
													<td>
														{ paternity.status === 1
														? (
<span
												className="truncate whitespace-nowrap inline-block py-1 px-3 rounded-full text-xs font-medium bg-success/10 text-success/80">Submitted</span>)													
												 : (<span
															className="truncate whitespace-nowrap inline-block py-1 px-3 rounded-full text-xs font-medium bg-danger/15 text-info/80">{paternity.status}</span>
												 )}
											</td>
					
											<td className="font-medium space-x-2 rtl:space-x-reverse">
												<div className="hs-tooltip ti-main-tooltip">
													<Link to={`${import.meta.env.BASE_URL}leaves/paternity/show_paternity_leave/${paternity.id}`}
                                                    ...
                                                    onClick={() => {
                                                   navigate(`${import.meta.env.BASE_URL}leaves/paternity/edit_paternity_leave/${paternity.id}`);   
                                                    }}
                                                   >
                                                    <i className="ti ti-pencil"></i>
                                                </button>			
										      &nbsp;&nbsp;														
											{paternity.status === null ? (
										<button
											type="button"
											className="ti-btn ti-btn-success show-example-btn"
											aria-label="Deactivate! Example: End of contract"
											id="ajax-btn"
											onClick={(e) => Ajaxcalling(e, paternity.id)}
										>
											Initiate
										</button>
										) : (
