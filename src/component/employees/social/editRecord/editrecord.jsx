...
import { JobTitleData, PackageData, RegionData, DistrictData, WardData, UsersData, DepartmentData, NationalityData, RankingCriterialData, EmployeeDependent, EmployeeRelative, DependantTypeData } from '/src/common/select2data';
...
const EditSocialRecord = () => {
    ...
    //fetch social_records data'
    const { id } = useParams();
    useEffect(() => {
        axios.get(`${apiBaseUrl}/employees/social/edit_social_record/${id}`).then((res) => {
            // Ensure that all properties are present in the API response
            const updatedFormData = {
                ...formData,
                ...res.data.social_records,
            };
    ...
    const handleSubmit = async (e) => {
        ...
        const DataToSend = {
            ...
            employee_id: id,
            social_signed_doc: formData.social_signed_doc,
        ...
        try {
            const resp = await axios.post(`${apiBaseUrl}/employees/social/update_social_record/${id}`, DataToSend, {
                headers: {
            ...
            if (resp.data.validator_err) {
                ...
                swal({
                    title: 'Employee Social Record Updated Successfully',
                    text: resp.data.message,
                    icon: 'success',
                    button: 'ok',
    ...
    function Style2() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire(
                    'Social Record Saved!',
                    'Social Record Updated Successfully.',
                    'success'
                ).then(() => {
    ...
    useEffect(() => {
        axios.get(`${apiBaseUrl}/employees/social/relative_detail/${id}`)
            .then((res) => {
                ...
                // console.log("dataa", ' ', res.data.relative_detail);
            })
            .catch((error) => {
    ...
    const handleFileRelativeInputChange = (index, fieldName, files) => {
    ...
    const handleRelativeInputChange = (index, field, value) => {
        setRelativeDetailData((prevData) => {
            // Make sure prevData[index] exists before accessing its properties
            if (prevData[index]) {
                return prevData.map((item, i) => {
    ...
    const handlerelativeChange = (selectedOption) => {
    ...
    const SaveRelativeDetail = async (e, relative) => {
        e.preventDefault();
        const relativeDetail = {
            social_record_id: formData.id,
            relative_id: relative.relative_id,
            relative_number: relative.relative_number,
            relationship_id: relative.relationship_id,
        ...
        try {
            const res = await axios.put(`${apiBaseUrl}/employees/social/update_relative_details/${id}`, relativeDetail);
            if (res.data.validator_err) {
                ...
                swal({
                    title: 'Relative Detail updated Successfully',
                    text: res.data.message,
                    icon: 'success',
                    button: 'ok',
                }).then(() => {
    ...
    const handleFileDependantInputChange = (index, fieldName, files) => {
    ...
    const handleDependantInputChange = (index, field, value) => {
        setDependantDetailData((prevData) => {
            // Make sure prevData[index] exists before accessing its properties
            if (prevData[index]) {
                return prevData.map((item, i) => {
    ...
    const saveDependantDetail = async (e, dependant) => {
        e.preventDefault();
        const DependantData = {

            dependant_name: dependant.dependant_name,
            employee_id: id,
            dependent_id: dependant.dependent_id,
            other_relationship: dependant.other_relationship,
            dob: dependant.dob,
            dependant_type_id: dependant.dependant_type_id,
            description: dependant.description,
            social_record_id: formData.id,

        }
        try {
            const res = await axios.put(`${apiBaseUrl}/employees/social/update_dependant_detail/${id}`, DependantData
            );
            if (res.data.validator_err) {
                // Handle validation errors
                const validationErrors = res.data.validator_err;
                ...
                swal({
                    title: 'Dependant details updated Successfully',
                    text: res.data.message,
                    icon: 'success',
                    button: 'ok',
                }).then(() => {
    ...
    return (
        <div>
            ...
            <div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h1 style={{ fontWeight: 'bold', fontSize: '2em', margin: 0 }}>Edit Employee Social Record </h1>
                ...
                <ol className="flex items-center whitespace-nowrap min-w-0 text-end">
                    ...
                    <li className="text-sm">
                        <a className="flex items-center text-primary hover:text-primary dark:text-primary" href={`${import.meta.env.BASE_URL}employees/socialrecords/edit_record/${formData.id}`}>
            ...
            <div className="box">
                <div className="box-header lg:flex lg:justify-between">
                    <h1 className="box-title my-auto font-bold text-lg">Update Employee Social Record</h1>
                        ...
                        {step === 1 && (
                            ...
                            <div className="grid lg:grid-cols-3 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="firstname" className="my-auto ti-form-input text-black" placeholder="Employee firstname" value={formData.firstname}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="middlename" className="my-auto ti-form-input text-black" placeholder="Middlename" value={formData.middlename}
                                    ...
                                    <input type="text" name="lastname" className="my-auto ti-form-input text-black" value={formData.lastname} onChange={(e) => handleInputChange('lastname', e.target.value)} placeholder="Employee Lastname" required />
                                ...
                                {formData.marital_status === '1' && (
                                    <>
                                        <div className="space-y-2">
                                            ...
                                            <input type="text" className="my-auto ti-form-input text-black" placeholder="Name of spouse" name="spause_name" value={formData.spause_name}
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <input type="number" className="my-auto ti-form-input text-black" placeholder="Number of children" name="children_no" value={formData.children_no}
                                ...
                                {formData.military_service === '1' && (
                                    <>
                                        <div className="space-y-2" id="military-number">
                                            ...
                                            <input type="number" name="military_number" className="my-auto ti-form-input text-black" value={formData.military_number} onChange={(e) => handleInputChange('military_number', e.target.value)} placeholder="Military number" required />
                                        </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex rounded-sm overflow-auto">
                                        ...
                                        <input
                                            type="date" name="expiration_date" className="my-auto ti-form-input text-black"
                        ...
                        {step === 2 && (
                            ...
                            <div className="grid lg:grid-cols-3 gap-6 second-page none" id="new_page">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="number" name="tin" className="my-auto ti-form-input text-black" value={formData.tin}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="email" name="person_email" className="my-auto ti-form-input text-black" value={formData.person_email}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="mobile_number" className="my-auto ti-form-input text-black" value={formData.mobile_number}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="telephone_home" className="my-auto ti-form-input text-black" value={formData.telephone_home}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="postal_address" className="my-auto ti-form-input text-black" value={formData.postal_address}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="employee_street" className="my-auto ti-form-input text-black" value={formData.employee_street}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input type="text" name="remark" className="my-auto ti-form-input text-black" value={formData.remark}
                                        onChange={(e) => handleInputChange('remark', e.target.value)} placeholder="ny Other Information for emergence" required />

                                </div>
                                <div className="space-y-2" id="attachment">
                                    <label className="ti-form-label mb-0 font-bold text-lg ">Social Record Signed (max size 2MB)</label>
                                    <input type="file" name="social_signed_doc" id="small-file-input" onChange={(e) => handleFileInputChange('social_signed_doc', e.target.files)} className="block w-full border border-gray-200 focus:shadow-sm dark:focus:shadow-white/10 rounded-sm text-sm focus:z-10 focus:outline-0 focus:border-gray-200 dark:focus:border-white/10 dark:border-white/10 dark:text-white/70 file:bg-transparent file:border-0 file:bg-gray-100 ltr:file:mr-4 rtl:file:ml-4 file:py-2 file:px-4 dark:file:bg-black/20 dark:file:text-white/70" />
                                    <span className="text-danger">{formData.error_list.social_signed_doc}</span>
                                </div>
                            </div>
                        )}
                            ...
                            {step === 2 && (
                                <div className="float-end">
                                    <button type="button" onClick={handleSubmit} className="ti-btn ti-btn-secondary  justify-center">
                                        <i className="ti ti-layout-grid-add"></i>Update Social record
                                    </button>
                                    <Link to="#" className="hs-dropdown-toggle py-2 px-3 ti-btn ti-btn-primary m-0 whitespace-nowrap" data-hs-overlay="#task-compose" style={{ backgroundColor: '#619162' }}>
                                        <i className="ti ti-database"></i>Update Relative details
                                    </Link>&nbsp;
                                    <Link to="#" className="hs-dropdown-toggle py-2 px-3 ti-btn ti-btn-primary m-0 whitespace-nowrap" data-hs-overlay="#dependent-people" >
                                        <i className="ti ti-user-check"></i>Update Dependent
                                    </Link>&nbsp;
                                ...
                                <div className="ti-modal-body">
                                    ...
                                    <div className="grid lg:grid-cols-3 gap-6">
                                                <div className="space-y-2">
                                                    ...
                                                    <Creatable classNamePrefix="react-select " className="text-black" name="relative_id" options={EmployeeRelative} onChange={(selectedOption) => handleRelativeInputChange(["relative_id"], selectedOption ? selectedOption.value : null)} value={EmployeeRelative.find((option) => option.value === relative.relative_id)} required />
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <input type="text" name="relative_name" className="ti-form-input text-black" placeholder="Relative Name" value={relativeData[index].relative_name}
                                                        onChange={(e) => handleRelativeInputChange(index,'relative_name', e.target.value)} required />
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <input type="text" name="relative_number" className="ti-form-input text-black" placeholder="Relative Mobile Number" value={relativeData[index].relative_number}
                                                        onChange={(e) => handleRelativeInputChange(index, 'relative_number', e.target.value)} required />
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Creatable classNamePrefix="react-select" name="relationship_id" options={relationships} onChange={(selectedOption) => handleRelativeInputChange(["relationship_id"], selectedOption ? selectedOption.value : null)} value={relationships.find((option) => option.value === relative.relationship_id)} />
                                                    {/* <span className="text-danger">{relativeData.error_list.relationship_id}</span> */}
                                                </div>
                                                {relativeData[index].relationship_id === 16 &&
                                                    (<div className="space-y-2">
                                                        <label className="ti-form-label mb-0 font-bold text-lg">Other Relationship</label>
                                                        <input type="text" name="other_relationship" className="ti-form-input text-black " placeholder="write Other if not listed" value={relativeData[index].other_relationship}
                                                            onChange={(e) => handleRelativeInputChange(index,'other_relationship', e.target.value)} required />
                                                    </div>)
                                                ...
                                                <div className="ti-modal-footer">
                                                    ...
                                                    <Link className="ti-btn ti-btn-primary float-right" to="#" onClick={(e) => SaveRelativeDetail(e, relative)}>
                                                        Create
                                                    </Link>
                                                </div>
                                                <br/>
                                ...
                                <div className="ti-modal-body">
                                    ...
                                    <div className="grid lg:grid-cols-3 gap-6">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Creatable classNamePrefix="react-select" name="dependent_id" options={EmployeeDependent} onChange={(selectedOption) => handleDependantInputChange(["dependent_id"], selectedOption ? selectedOption.value : null)} value={EmployeeDependent.find((option) => option.value === dependant.dependent_id)} />
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <input type="text" name="dependant_name" className="ti-form-input text-black" placeholder="Dependent Name" value={dependantData[index].dependant_name}
                                                        onChange={(e) => handleDependantInputChange(index,'dependant_name', e.target.value)} required />
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Creatable classNamePrefix="react-select" name="dependant_type_id" options={relationships} onChange={(selectedOption) => handleDependantInputChange(["dependant_type_id"], selectedOption ? selectedOption.value : null)} value={relationships.find((option) => option.value === dependant.dependant_type_id)} />
                                                ...
                                                {dependantData.dependant_type_id === 16 && (
                                                    <div className="space-y-2">
                                                        ...
                                                        <input type="text" name="other_relationship" className="ti-form-input text-black" placeholder="Dependent relation if not listed" value={dependantData[index].other_relationship}
                                                            onChange={(e) => handleDependantInputChange(index,'other_relationship', e.target.value)} required />
                                                    </div>
                                                )}
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex rounded-sm overflow-auto">
                                                        ...
                                                        <input
                                                            type="date" name="dob" className="my-auto ti-form-input text-black"
                                                            placeholder="" value={new Date(dependantData[index].dob).toLocaleDateString('en-CA')} // Format the date
                                                            onChange={(e) => handleDependantInputChange(index,'dob', e.target.value)} required
                                                        />
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <input type="text" name="description" className="ti-form-input text-black" placeholder="Note or Remark " value={dependantData[index].description}
                                                        onChange={(e) => handleDependantInputChange(index,'description', e.target.value)} required />
                                                </div>
                                                ...
                                                <div className="ti-modal-footer">
                                                    ...
                                                    <Link className="ti-btn ti-btn-primary" to="#" onClick={(e) => saveDependantDetail(e, dependant)}>
                                                        Create
                                                    </Link>
                                                </div>
                                            </div>
...
export default EditSocialRecord;
