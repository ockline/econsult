...
const ShowAssessment = () => {
    ...
    // /**   Block for document preview  */
     const [assessedDocument, setAssessedDocument] = useState([]);
  const [documentUrl, setDocumentUrl] = useState('');

  useEffect(() => {
    axios.get(`${apiBaseUrl}/hiring/hr_interview/get_assessed_document/${id}`)
      .then((res) => {
          setAssessedDocument(res.data.assessed_document);
          console.log(res.data.assessed_document);
      })
      .catch((error) => {
        console.error('Error fetching assessed documents:', error);
      });
  }, [id]);

  const handlePreviewClick = (description) => {
    // Assuming the documents are stored in a specific folder on the server      
      const absoluteUrl = `${docBaseUrl}/hiring/hrInterview/${description}`;
//   console.log('absoluteUrl', absoluteUrl);
  ...
  setDocumentUrl(absoluteUrl);
   ...
   <div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
				<h1 style={{ fontWeight: 'bold', fontSize: '2em', margin: 0 }}>Assessed Candidate Details</h1>
				...
				<ol className="flex items-center whitespace-nowrap min-w-0 text-end">
					...
					<li className="text-sm">
					<a className="flex items-center text-primary hover:text-primary dark:text-primary" href={`${import.meta.env.BASE_URL}hiring/recruitments/hr/show_assessment/${formData.id}`}>Assessed Candidate Details
						{/* <i className="ti ti-chevrons-right flex-shrink-0 mx-3 overflow-visible text-gray-300 dark:text-white/10 rtl:rotate-180"></i> */}
					</a>
					</li>
				</ol>
							...
							<div className="pb-5">
								<div className="md:flex justify-between space-y-2 md:space-y-0">
									...
									<div className="md:ltr:ml-auto md:rtl:mr-auto">
										<Link to={`${import.meta.env.BASE_URL}hiring/hrinterview/download_assessment/` + formData.id} className="ti-btn  bg-success text-xs m-0 ti-btn-soft p-2 font-semibold !text-gray-500 !text-white/70"><i className="ti ti-download"></i>Download Assessment</Link>
									</div>
								</div>
							</div>
								<div className="overflow-auto">
								<table className="ti-custom-table  table-bordered ti-custom-table-head">
									...
									<tbody>
								{Object.keys(assessedDocument).length > 0 ? (
                                Object.values(assessedDocument).map((document, index) => (
                                            <tr key={document.id}>
                                                <td>{index + 1}</td>
