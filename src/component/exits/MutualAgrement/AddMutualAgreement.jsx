...
const AddMutualAgreement = () => {
  ...
  const handleSubmit = async (values, action = 'save') => {
    ...
    try {
      ...
      submitFormData.append('exit_type', 'mutual_agreement');
      ...
      // Use mutual agreement endpoints
      const endpoint = action === 'submit' 
        ? `${apiBaseUrl}/employees/exits/mutual_agreement/submit_mutual_agreement`
        : `${apiBaseUrl}/employees/exits/mutual_agreement/create_mutual_agreement`;
      ...
      if (response.data.status === 200) {
        ...
        Swal.fire({
          title: 'Success',
          text: `Mutual agreement ${actionText} successfully`,
          icon: 'success',
          confirmButtonText: 'OK',
          confirmButtonColor: '#b2000a'
        }).then(() => {
          navigate(`${import.meta.env.BASE_URL}exits/mutual_agreements`);
        });
      } else {
        message.error(response.data.message || `Failed to ${action} mutual agreement`);
      }
    } catch (error) {
      console.error(`Error ${action}ing mutual agreement:`, error);
      message.error(error.response?.data?.message || `Failed to ${action} mutual agreement`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontWeight: 'bold', fontSize: '2em', margin: 0 }}>Add Mutual Agreement</h1>
        ...
        <div className="box-body">
          ...
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold">
              {currentStep === 1 && 'Step 1: Mutual Agreement Contract Details'}
              ...
              <Card 
                title={
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span>Mutual Agreement Contract Details</span>
                    <button 
                      type="button" 
                      className="ti-btn ti-btn-primary" 
                      style={{ backgroundColor: '#b2000a', borderColor: '#b2000a' }}
                      onClick={() => navigate(`${import.meta.env.BASE_URL}exits/mutual_agreements`)}
                    >
...
export default AddMutualAgreement;
