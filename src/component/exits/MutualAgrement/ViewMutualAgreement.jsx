...
const ViewMutualAgreement = () => {
  ...
  const fetchEndContract = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${apiBaseUrl}/employees/exits/mutual_agreement/show_mutual_agreement/${id}`,
        {
          headers: {
      ...
      if (response.data.status === 200) {
        ...
        navigate('/exits/mutual_agreements');
      }
    } catch (error) {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const response = await axios.get(
        `${apiBaseUrl}/employees/exits/mutual_agreement/get_attachments/${endContract.id}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const generatePDF = async () => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const endpoint = `${apiBaseUrl}/employees/exits/mutual_agreement/generate_mutual_agreement_pdf/${id}`;
      ...
      setGeneratedPdf({ blob, url, name: `mutual_agreement_${endContract.employee_name}_${dayjs().format('YYYY-MM-DD')}.pdf` });
  ...
  const savePDF = async () => {
    ...
    try {
      ...
      formData.append('document_name', `Mutual Agreement - ${endContract.employee_name}`);
      formData.append('document_type', 'mutual_agreement');

      const response = await axios.post(
        `${apiBaseUrl}/employees/exits/mutual_agreement/save_attachment/${id}`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  if (loading && !endContract) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px', flexDirection: 'column' }}>
        ...
        <p style={{ marginTop: '20px', fontSize: '16px', color: '#666' }}>Loading mutual agreement details...</p>
      </div>
    );
  }
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px', flexDirection: 'column' }}>
        ...
        <button
          ...
          onClick={() => navigate('/exits/mutual_agreements')}
        >
  ...
  return (
    <div>
      ...
      <div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontWeight: 'bold', fontSize: '2em', margin: 0 }}>View Mutual Agreement</h1>
                ...
                <div className="box">
                  ...
                  <div className="box-body">
                    ...
                    <div className="mb-6 p-4 border rounded-lg bg-gray-50">
                      <h6 className="text-lg font-semibold mb-4">Generate Mutual Agreement PDF</h6>
                      <Button
                        type="primary"
                      ...
                      <p className="text-sm text-gray-600 mt-2">
                        Click to generate and preview the filled Mutual Agreement document. You can save it after reviewing.
                      </p>
                    </div>

                    {/* PDF Preview Modal */}
                    <Modal
                      title="Mutual Agreement - PDF Preview"
...
export default ViewMutualAgreement;
