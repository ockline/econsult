...
const AddRetrenchment = () => {
  ...
  const handleSubmit = async (values, action = 'save') => {
    ...
    try {
      ...
      submitFormData.append('exit_type', 'retrenchment');
      ...
      // Use retrenchment endpoints
      const endpoint = action === 'submit' 
        ? `${apiBaseUrl}/employees/exits/retrenchment/submit_retrenchment`
        : `${apiBaseUrl}/employees/exits/retrenchment/create_retrenchment`;
      ...
      if (response.data.status === 200) {
        ...
        Swal.fire({
          title: 'Success',
          text: `Retrenchment ${actionText} successfully`,
          icon: 'success',
          confirmButtonText: 'OK',
          confirmButtonColor: '#b2000a'
        }).then(() => {
          navigate(`${import.meta.env.BASE_URL}exits/retrenchments`);
        });
      } else {
        message.error(response.data.message || `Failed to ${action} retrenchment`);
      }
    } catch (error) {
      console.error(`Error ${action}ing retrenchment:`, error);
      message.error(error.response?.data?.message || `Failed to ${action} retrenchment`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontWeight: 'bold', fontSize: '2em', margin: 0 }}>Add Retrenchment</h1>
        ...
        <div className="box-body">
          ...
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold">
              {currentStep === 1 && 'Step 1: Retrenchment Contract Details'}
              ...
              <Card 
                title={
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span>Retrenchment Contract Details</span>
                    <button 
                      type="button" 
                      className="ti-btn ti-btn-primary" 
                      style={{ backgroundColor: '#b2000a', borderColor: '#b2000a' }}
                      onClick={() => navigate(`${import.meta.env.BASE_URL}exits/retrenchments`)}
                    >
...
export default AddRetrenchment;
