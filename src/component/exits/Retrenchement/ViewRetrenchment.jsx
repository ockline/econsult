...
const ViewRetrenchment = () => {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const response = await axios.get(
        `${apiBaseUrl}/employees/exits/retrenchment/get_attachments/${endContract.id}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const generatePDF = async () => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const endpoint = `${apiBaseUrl}/employees/exits/retrenchment/generate_retrenchment_pdf/${id}`;
      ...
      setGeneratedPdf({ blob, url, name: `retrenchment_${endContract.employee_name}_${dayjs().format('YYYY-MM-DD')}.pdf` });
  ...
  const savePDF = async () => {
    ...
    try {
      ...
      formData.append('document_name', `Retrenchment - ${endContract.employee_name}`);
      formData.append('document_type', 'retrenchment');

      const response = await axios.post(
        `${apiBaseUrl}/employees/exits/retrenchment/save_attachment/${id}`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handlePreviewClick = async (attachment) => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const attachmentUrl = `${apiBaseUrl}/employees/exits/retrenchment/get_attachment_file/${endContract.id}/${attachment.id}`;
  ...
  if (loading && !endContract) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px', flexDirection: 'column' }}>
        ...
        <p style={{ marginTop: '20px', fontSize: '16px', color: '#666' }}>Loading retrenchment details...</p>
      </div>
    );
  }
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px', flexDirection: 'column' }}>
        ...
        <button
          ...
          onClick={() => navigate('/exits/retrenchments')}
        >
  ...
  return (
    <div>
      ...
      <div className="box-body" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontWeight: 'bold', fontSize: '2em', margin: 0 }}>View Retrenchment</h1>
                ...
                <div className="box">
                  ...
                  <div className="box-body">
                    ...
                    <div className="mb-6 p-4 border rounded-lg bg-gray-50">
                      <h6 className="text-lg font-semibold mb-4">Generate Retrenchment PDF</h6>
                      <Button
                        type="primary"
                      ...
                      <p className="text-sm text-gray-600 mt-2">
                        Click to generate and preview the filled Retrenchment document. You can save it after reviewing.
                      </p>
                    </div>

                    {/* PDF Preview Modal */}
                    <Modal
                      title="Retrenchment - PDF Preview"
...
export default ViewRetrenchment;
